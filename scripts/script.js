...
async function main() {
  ...
  const setSubContractTx = await nfct.commitEncryptedCode(1, encryptedCode);
  ...
  const deploySubContractTx = await nfct.decryptAndDeployCode(1, key);
  await deploySubContractTx.wait();
  const runSubContractTx = await nfct.runCode(1, "append(string,string)", ["Hola, ", "baby!"]);
